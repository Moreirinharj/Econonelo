"""IA que controla o mundo."""
import random
import database as db
from utils.logger import log_acao

# Todos os templates agora têm EXATAMENTE 3 placeholders {}
TEMPLATES_NOTICIAS = {
    "economia": [
...
def _preencher_template(template: str, categoria: str) -> str:
    """Preenche 3 placeholders de forma coerente com a categoria."""
...
def avancar_dia_mundo():
    dia_atual_str = db.obter_estado_mundo("dia")
    try:
        dia_atual = int(dia_atual_str)
    except (ValueError, TypeError):
        # Se não for um número válido, inicializar com 1
        dia_atual = 1
        db.atualizar_estado_mundo("dia", str(dia_atual))
